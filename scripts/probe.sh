...
# host filesystem outside the working directory.
set -euo pipefail
...
probe_docker() {
    ...
    case "$info" in
        *"/arm64"|*"/aarch64") ok "docker daemon: $info" ;;
        *) fail "docker daemon architecture is not ARM64 (got $info)" ;;
    esac
}
